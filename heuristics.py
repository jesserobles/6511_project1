...
def largest_bucket_first_heuristic2(state: Tuple[int], capacities: Tuple[int], target: int) -> int:
    delta = target - state[-1]
    space_available = [capacity - bucket for capacity, bucket in zip(capacities[:-1], state[:-1])]
    if delta == 0: # This is a solution state
        return 0
    ...
    h = max(abs(delta)/volumetric_potential_of_current_state * steps_required_to_repeat_state, 1)
